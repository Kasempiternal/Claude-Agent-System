...
Agent OS creates configuration files directly in your project:
- `.eslintrc.js`, `.prettierrc` - Linting/formatting configs
- `tsconfig.json`, `pyproject.toml` - Language configs
- CI/CD pipelines in `.github/workflows/` or `.gitlab-ci.yml`

Temporary workflow files go to `~/.claude/temp/` and are deleted after completion.
Documentation is only created when explicitly requested by the user.

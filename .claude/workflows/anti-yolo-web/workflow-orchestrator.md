...
- Save wireframe to ~/.claude/temp/wireframes/
...
def execute_anti_yolo_workflow(optimized_prompt, project_context):
    ...
    save_wireframe(wireframe, "~/.claude/temp/wireframes/")
...
### Workflow Files
- Wireframes: `~/.claude/temp/wireframes/[task-name]-[timestamp].txt`
- Implementation notes: `~/.claude/temp/implementation/[task-name]/`
- Test results: `~/.claude/temp/tests/anti-yolo/[task-name]/`

### Pattern Learning
- Track successful wireframe patterns in current session
- Record user preferences for current workflow
- Save implementation approaches that worked well
- Note any issues encountered and solutions applied

**IMPORTANT**: These are temporary workflow files. Permanent documentation only created when user explicitly requests it.

...
3. **Session Wrap-up**
   - Complete current workflow tasks
   - Document any critical issues found
   - Prepare handoff notes (only if user requests documentation)
...
### With Workflow State
- Updates temporary workflow state in ~/.claude/temp/
- Documents patterns discovered (only if user requests permanent docs)
- Records critical decisions (only if user requests permanent docs)
- Captures solutions for current session

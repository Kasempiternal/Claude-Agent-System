#!/usr/bin/env node
/**
 * Dangerous Commands - PreToolUse Hook for Bash
 * Intercepts dangerous shell commands and escalates to user confirmation.
 ...
 *     "PreToolUse": [{
 *       "matcher": "Bash",
 *       "hooks": [{ "type": "command", "command": "node /path/to/dangerous-commands.js" }]
 *     }]
 *   }
 * }
 */
...
function log(data) {
  try {
    ...
    fs.appendFileSync(file, JSON.stringify({ ts: new Date().toISOString(), hook: 'dangerous-commands', ...data }) + '\n');
  } catch {}
}
...
async function main() {
  ...
  try {
    ...
    if (result.blocked) {
      ...
      log({ level: 'ASK', id: p.id, priority: p.level, cmd, session_id, cwd, permission_mode });
      return console.log(JSON.stringify({
        hookSpecificOutput: {
          hookEventName: 'PreToolUse',
          permissionDecision: 'ask',
          permissionDecisionReason: `${EMOJIS[p.level]} [${p.id}] ${p.reason} — approve to proceed`
        }
      }));
    }
    console.log('{}');

...
1. **Master task list**: Read `.cas/plans/legion-{slug}/project-tasks.md` for the full project state
...
5. **Prior wave state** (if Wave > 1): Read `.cas/plans/legion-{slug}/wave-{I}-{W-1}-state.md`
...
{If Wave > 1:}
PRIOR WAVE STATE FILE: .cas/plans/legion-{slug}/wave-{I}-{W-1}-state.md
...
- For Iteration 2+: read `.cas/plans/legion-{slug}/wave-{I-1}-{last}-state.md` for what was built
- For Wave 2+: read `.cas/plans/legion-{slug}/wave-{I}-{W-1}-state.md` for files modified
